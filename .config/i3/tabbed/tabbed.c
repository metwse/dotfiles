...
static int use_wid;
static int wid;
static Bool not_draw_bar = False;
static char* startup_command = "true";
...
setup(void)
{
    ...
    if (use_wid) win = wid; 
    else win = XCreateSimpleWindow(dpy, root, wx, wy, ww, wh, 0, dc.norm[ColFG].pixel, dc.norm[ColBG].pixel);
